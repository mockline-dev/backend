...
describe('buildTree', () => {
  it('builds a tree with correct file count', () => {
    const tree = buildTree('proj1', makeFiles([['src/a.ts', 'aaa'], ['src/b.ts', 'bbb']]))
  ...
  it('produces same root hash for same content regardless of input order', () => {
    const t1 = buildTree('p', makeFiles([['a.ts', 'aa'], ['b.ts', 'bb']]))
    const t2 = buildTree('p', makeFiles([['b.ts', 'bb'], ['a.ts', 'aa']]))
...
describe('diffTrees', () => {
  it('treats all files as added when oldTree is null', () => {
    const newTree = buildTree('p', makeFiles([['a.ts', 'aa'], ['b.ts', 'bb']]))
  ...
  it('detects deleted files', () => {
    const old = buildTree('p', makeFiles([['a.ts', 'aa'], ['b.ts', 'bb']]))
  ...
  it('detects added files', () => {
    ...
    const next = buildTree('p', makeFiles([['a.ts', 'aa'], ['b.ts', 'bb']]))
    const diff = diffTrees(old, next)
    expect(diff.added).toContain('b.ts')
    expect(diff.unchanged).toBe(1)
  })
...
describe('updateTree', () => {
  ...
  it('removes deleted paths', () => {
    const tree = buildTree('p', makeFiles([['a.ts', 'aa'], ['b.ts', 'bb']]))
